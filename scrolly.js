...
function createScrollyContentFromCSVFile() {
  Papa.parse("./scrolly-step-data.csv", {
    header: true,
    delimiter: ",",
    download: true,
    complete: createScrollyContentFromCSVData,
    error: function (error, file) {
      displayErrors([{ Error: `Parse File: ${error}` }]);
    },
  });
}

function createScrollyContentFromCSVData(allStepData) {
  if (isParseError(allStepData.errors)) {
    displayErrors(allStepData.errors);
    return;
  }

  var stepNumber = 1;
  allStepData.data.forEach((stepData) => {
...
function isParseError(parseErrors, papaErrors) {
  return parseErrors == null || parseErrors.length > 0;
}
